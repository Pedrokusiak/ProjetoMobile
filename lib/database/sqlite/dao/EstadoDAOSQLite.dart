import 'package:projeto03/view/interface/EstadoInterfaceDAO.dart';
...
class EstadoDAOSQLite implements EstadoInterfaceDAO {
  ...
  Future<Estado> consultar(int id) async {
    ...
    Map resultado =
        (await db.query('estado', where: 'id = ?', whereArgs: [id])).first;
  ...
  Future<List<Estado>> consultarTodos() async {
    Database db = await Conexao.criar();
    List<Estado> lista =
        (await db.query('estado')).map<Estado>(converter).toList();
    return lista;
  }
  ...
  Future<Estado> salvar(Estado estado) async {
    ...
    if (estado.id == null) {
      ...
      estado = Estado(id: id, nome: estado.nome, sigla: estado.sigla);
    } else {
